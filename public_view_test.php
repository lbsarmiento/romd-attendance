...
<head>
    ...
    <title>ROMD Attendance - Public View Test</title>
    <style>
        * {
...
<body>
    <div class="test-shell">
        <header class="test-header">
            <a href="public_view_test.php" class="test-brand">
                ...
                <span class="test-brand-text">
                    <span class="test-brand-title">ROMD Attendance</span>
                    <span class="test-brand-subtitle">Public View Test Draft</span>
                </span>
            </a>
            <div class="test-actions">
                <a href="javascript:void(0)" class="test-btn test-btn-secondary" id="refreshSelectedBtn">Refresh Day</a>
                <a href="landingpage.php" class="test-btn test-btn-secondary">Open Live Public View</a>
                <?php if ($is_logged_in): ?>
        ...
        <main class="test-main">
            <section class="hero-panel">
                <div class="hero-grid">
                    <div>
                        <h1 class="hero-title">Working-day public deck draft.</h1>
                        <p class="hero-copy">
                            This test page copies the public-view style and adds a rectangle slide deck for working days only. Click a date card to switch days with an animated transition, while hover states shimmer for a more lively presentation.
                        </p>
                        <div class="hero-meta">
                            <div class="hero-chip">
                                <div class="hero-chip-label">Current Month Deck</div>
                    ...
                    <div class="highlight-card">
                        <div>
                            <div class="highlight-badge">Early Bird</div>
                        ...
                        <div class="highlight-caption" id="highlightCaption">
                            Select a working day card below to preview the animated public board transition.
                        </div>
                    </div>
                </div>
            </section>
            ...
            <section class="deck-panel glass-card">
                <div class="deck-header">
                    <div>
                        <h2 class="deck-title">Working Day Slide Deck</h2>
                        <p class="deck-copy">Rectangle date cards show only Monday to Friday for the current month.</p>
                    </div>
                ...
                <div class="board-stage" id="boardStage">
                    <div class="board-top">
                        <div class="board-date-card">
                            <h2 class="board-date-title" id="boardDateTitle">Loading...</h2>
                            <div class="summary-pills" id="summaryPills"></div>
                        </div>
                    </div>
                    <div class="timeline-list" id="timelineList">
                        <div class="empty-state">
                            <h3>Loading working-day draft</h3>
                            <p>Please wait while the draft page retrieves attendance data for the selected date.</p>
                        </div>
                    </div>
                </div>
            </section>
        ...
        <footer class="test-footer">
            ...
            <span>Draft concept for working-day deck animation</span>
        </footer>
    </div>
    ...
    <script>
        ...
        function buildWorkingDays() {
            ...
            document.getElementById('deckMonthLabel').textContent = selected.toLocaleDateString('en-US', {
                month: 'long',
                year: 'numeric'
            });
        }
        ...
        function renderDeck() {
            ...
            deck.innerHTML = state.workingDays.map((dateStr) => `
                <button type="button" class="deck-card ${dateStr === state.selectedDate ? 'is-selected' : ''}" data-date="${dateStr}">
        ...
        function renderEmptyState(message) {
            document.getElementById('boardDateTitle').textContent = formatDisplayDate(state.selectedDate);
            ...
            document.getElementById('highlightCaption').textContent = 'Select a working day card below to preview the animated public board transition.';
        }
        ...
        function renderBoard(data) {
            ...
            document.getElementById('boardDateTitle').textContent = formatDisplayDate(data.date);
            ...
            if (earlyBird) {
                const statusClass = getStatusClass(earlyBird.status);
                document.getElementById('highlightName').textContent = earlyBird.name || 'Early Bird';
                document.getElementById('highlightTime').textContent = formatTime(earlyBird.time_in);
                document.getElementById('highlightCaption').textContent = `${getStatusLabel(statusClass)} and first recorded time-in for the selected day.`;
            } else {
            ...
            document.getElementById('timelineList').innerHTML = employees.map((employee, index) => {
                ...
                return `
                    <div class="timeline-row status-${statusClass} ${isEarly ? 'is-early' : ''}" style="animation-delay:${index * 0.04}s;">
                        <div class="timeline-rank">${index + 1}</div>
                        <div>
            ...
            window.setTimeout(() => {
                fetch(`get_public_today.php?date=${encodeURIComponent(dateStr)}`, { cache: 'no-store' })
                    ...
                    .catch(() => {
                        ...
                        renderEmptyState('The draft board could not load that day right now. Please try another date or refresh again.');
                    })
                    .finally(() => {
                        if (requestId === state.requestId) {
        ...
        function moveSelectedDay(step) {
            ...
            if (nextIndex < 0 || nextIndex >= state.workingDays.length) {
                return;
            }

            selectDate(state.workingDays[nextIndex]);
        }
